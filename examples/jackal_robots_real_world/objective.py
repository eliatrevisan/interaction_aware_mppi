...
class JackalRobotObjective(object):
    def __init__(self, goals, device="cuda:0"):
        self.nav_goals = torch.tensor(goals, device=device).unsqueeze(0)
        self._goal_weight = 1.0
        self._back_vel_weight = 0.0
        self._rot_vel_weight = 0.3

    def compute_running_cost(self, state: torch.Tensor):
        goal_dist = self._goal_cost(state[:, :, 0:2])
        back_vel = self._backward_vel_cost(state)
        return goal_dist * self._goal_weight + back_vel * self._back_vel_weight + self._rot_vel_cost(state) * self._rot_vel_weight
    
    def _goal_cost(self, positions):
        return torch.linalg.norm(positions - self.nav_goals, axis=2)
    
    def _backward_vel_cost(self, state):
        ...
        return torch.relu(-vel_body[:, :, 0])
    
    def _rot_vel_cost(self, state):
        return state[:, :, 5] ** 2
